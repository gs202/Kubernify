...
class TestDiscoverClusterStateCacheLifecycle:
    ...
    def test_caches_are_seeded_and_cleared_even_on_inspect_failure(self) -> None:
        ...
        with patch.object(discovery, "inspect_workload", side_effect=RuntimeError("boom")):
            discovery.discover_cluster_state(namespace="foo-ns")

        controller.seed_deployment_replica_set_cache.assert_called_once_with(namespace="foo-ns")
        controller.clear_deployment_replica_set_cache.assert_called_once_with(namespace="foo-ns")
        controller.seed_namespace_pod_cache.assert_called_once_with(namespace="foo-ns")
        controller.clear_namespace_pod_cache.assert_called_once_with(namespace="foo-ns")
